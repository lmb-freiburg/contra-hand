...
from manopth.load_util import ready_arguments
...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        """
        Args:
            center_idx: If number, then this is considered to be the keypoint 
                index to center of. Defaults to None in which case centering
                is skipped.
    ...
    def forward(self,
        ...
        # Possibly center on a certain keypoint
        if self.center_idx is not None:
            center_joint = th_jtr[:, self.center_idx].unsqueeze(1)
            th_jtr = th_jtr - center_joint
            th_verts = th_verts - center_joint
        th_jtr = th_jtr + th_trans
        th_verts = th_verts + th_trans

        return th_verts, th_jtr

    def calc_alphas(self, pose):
        if self.flat_hand_mean:
            pose = pose - self.th_hands_mean.squeeze(0)

        alphas = list()
        for comp in self.th_selected_comps:
            v = torch.sqrt(torch.sum(torch.square(comp)) + 1e-8)
            alphas.append( torch.sum(torch.mul(comp, pose))/v/v )
